...
--Oppgave 4
select f.title, count(*) as num_genre, fi.filmtype
...
select am.country, ant_filmer, avg_rating, genre
...
from filmitem as fi
    join filmcountry as fc on (fi.filmid = fc.filmid)
    join filmparticipation as fp on (fi.filmid = fp.filmid)
    join person as p on (p.personid = fp.personid)

    join filmparticipation as fp2 on (fi.filmid = fp2.filmid)
...
from film as f 
    full outer join filmgenre as fg on (f.filmid = fg.filmid) 
    full outer join filmcountry as fc on (f.filmid = fc.filmid) 
where (f.title like '%Dark%' or f.title like '%Night%') and (fg.genre = 'Horror' or fc.country = 'Romania');

--Oppgave 8
select f.title, count(*) as ant_del
from film as f
